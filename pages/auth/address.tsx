...
import { ILocation, IProfile, IUser } from '../../interfaces';
...
function UserAddress() {
  ...
  const countrychange = (values: IProfile, country: string) => {
    if (country == 'Rwanda') {
  ...
  const createMutation = useMutation(UpdateUser);

  const handleUpdateUser = async (payload: IProfile) => {
    ...
    payload.user_picture = user?.profile?.user_picture;
    ...
    setIsSaving(true);
    const updatedUser = await createMutation.mutateAsync({ ...payload })
    if (updatedUser) {
      setIsSaving(false);
      console.log(updatedUser);
      if (updatedUser.status) {
        setSuccessMsg("Profile updated successfully! Next, complete your business details.");
        let newuser: IUser = { ...user! };
        newuser.profile = updatedUser?.profile;
        setUser(newuser);
        localStorage.setItem('user', JSON.stringify(newuser));
        setTimeout(() => { Router.push("/auth/create-shop") }, 3000);
      }
      else {
        setErrorMsg(updatedUser.message);
      }
    }
  ...
  return (
    <div className="main-content auth-container">
      <div className="auth-description">
        <h1 className="text-grey my-4">
          {(user && user.profile?.gender == "Male") ? ("Mr.") : ((user && user.profile?.gender == "Female") ? ("Ms.") : ("Mx."))}
          {user && user.profile?.first_name}, You&#39;re Almost there!
        </h1>
        <h4 className="text-grey font-weight-normal">
          For better services such as delivery experience, we would like to know the address of our beloved clients!
        </h4>
      </div>
                ...
                <Form role="form" onSubmit={handleSubmit}>
                  ...
                  <div className="text-center">
                    <Button className="my-4 w-100 bg-success text-white" type="submit">
                      {isSaving ? ("Saving Your Address Details...") : ("Next")}
                    </Button>
                  </div>
                </Form>
              )}
